...
const PriceTag: Component<{
    ...
    const has_discount =
        ep.kind === "Profile" &&
        ep.asProfile().special_price &&
        ep.asProfile().total_price > (ep.asProfile().special_price as number);

    return (
        <>
            <Show when={has_discount}>
                <span class={props.style.discount}>{ep.asProfile().total_price.toFixed(2)}</span>
            </Show>
